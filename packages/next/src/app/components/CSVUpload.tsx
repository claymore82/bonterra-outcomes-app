...
export default function CSVUpload({
  ...
  const parseCSV = useCallback((text: string) => {
    ...
    const hasFamilyName = headers.some(
      (h) =>
        h.toLowerCase().includes('family') && h.toLowerCase().includes('name'),
    );
    const hasRelationship = headers.some((h) =>
      h.toLowerCase().includes('relationship'),
    );
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length !== expectedColumnCount) {
        rowsWithIssues++;
        if (rowsWithIssues <= 3) {
          // Only report first 3 issues
          warnings.push(
            `⚠️ Row ${i + 1}: Expected ${expectedColumnCount} columns, found ${values.length}. Check for unquoted commas.`,
          );
        }
    ...
    if (rowsWithIssues > 3) {
      warnings.push(
        `⚠️ ${rowsWithIssues - 3} more rows have formatting issues`,
      );
    }
    ...
    if (rows.length === 0) {
      throw new Error(
        'No valid data rows found after parsing. Please check your CSV format.',
      );
    }
  ...
  return (
    <div>
      <div
        ...
        onClick={() =>
          !disabled && document.getElementById('csv-upload-intake')?.click()
        }
      >
        <input
          type="file"
        ...
        {error && (
          ...
          >
            <Stack space="200">
              <Text weight="600" style={{ color: '#dc2626' }}>
                ❌ CSV Format Error
              </Text>
              <Text
                variant="sm"
                style={{ color: '#991b1b', whiteSpace: 'pre-wrap' }}
              >
                {error}
              </Text>
              <div
              ...
              >
                ...
                <Text variant="sm" style={{ color: '#92400e' }}>
                  • Make sure fields with commas are wrapped in quotes
                  <br />
                  • Example: "123 Main St, Baltimore MD 21201"
                  <br />
                  • Ensure all rows have the same number of columns
                  <br />• Include "First Name" and "Last Name" columns
                </Text>
              </div>
            </Stack>
          </div>
      ...
      >
        <Text weight="600" style={{ marginBottom: '8px', fontSize: '14px' }}>
          Expected CSV Format:
        </Text>
        <Text
          variant="sm"
