...
import { ServiceType, SERVICE_CATEGORIES, SERVICE_UNITS } from '@/types/services';
import PageLayout from '../../components/PageLayout';

export default function ServicesPage() {
  const { serviceTypes, addServiceType, updateServiceType, deleteServiceType } = useServiceStore();
  ...
  const filteredServices = selectedProgramFilter === 'all'
    ? serviceTypes
  ...
  return (
    <PageLayout>
      <Stack space="600">
        ...
        <Card>
          <Stack space="300">
            ...
            <Select
              ...
              onSelectionChange={(key) => setSelectedProgramFilter(key as string)}
            >
        ...
        <Card>
          <Stack space="400">
            <Heading level={2}>Service Types ({filteredServices.length})</Heading>
              ...
              <Stack space="300">
                {filteredServices.map((service) => (
                  <Card key={service.id}>
                    <Stack space="300">
                      <div>
                        <Text weight="600" style={{ fontSize: '16px' }}>{service.name}</Text>
                        {service.description && (
                          <Text variant="sm" style={{ marginTop: '4px' }}>{service.description}</Text>
                        )}
                      </div>
                      ...
                      <InlineStack gap="300" verticalAlign="center">
                        ...
                        {service.costPerUnit && (
                          <Text variant="sm" color="subdued">
                            ${service.costPerUnit} per {SERVICE_UNITS[service.unit].toLowerCase()}
                          </Text>
                        )}
                      </InlineStack>
...
function ServiceModal({ service, onClose, onSave }: ServiceModalProps) {
  ...
  const [category, setCategory] = useState<string>(service?.category || 'case-management');
  const [costPerUnit, setCostPerUnit] = useState(service?.costPerUnit?.toString() || '');
            ...
            <Stack space="400">
              ...
              <div>
                <div style={{
                  ...
                  Programs * <span style={{ color: '#ef4444' }}>({selectedPrograms.size} selected)</span>
                </div>
                <div style={{
                  ...
                  {programs.filter(p => p.status === 'active').length === 0 ? (
                    <Text variant="sm" color="subdued">No active programs available</Text>
                  ) : (
                    <Stack space="200">
                      {programs.filter(p => p.status === 'active').map(program => (
                        <label
                          ...
                          style={{
                            ...
                            borderRadius: '6px',
                            cursor: 'pointer',
                            backgroundColor: selectedPrograms.has(program.id) ? 'white' : 'transparent',
                            border: selectedPrograms.has(program.id) ? '1px solid #7c3aed' : '1px solid transparent',
                          }}
                        >
                          <input
                            type="checkbox"
                          ...
                          />
                          <div style={{ flex: 1 }}>
                            <Text variant="sm" weight="500">{program.name}</Text>
                            {program.description && (
                            ...
                            {program.outcomeGoals && program.outcomeGoals.length > 0 && (
                              <Text variant="xs" color="subdued" style={{ marginTop: '4px' }}>
                                Outcomes: {program.outcomeGoals.slice(0, 2).join(', ')}
                                {program.outcomeGoals.length > 2 && ` +${program.outcomeGoals.length - 2} more`}
                              </Text>
                            )}
                          </div>
                        </label>
