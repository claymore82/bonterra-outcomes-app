...
import { HouseholdMember, RELATIONSHIP_LABELS, RelationshipType } from '@/types/household';
...
export default function CreateFamilyAgentPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: "Hello! I'll help you create a new family. You can tell me about the family in natural language, like 'I want to add the Smith family - John (father), Maria (mother), and their kids Sofia (8) and Diego (5).' What family would you like to add?",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleCreateFamily = () => {
    ...
    try {
      ...
      // Create household
      const household = createHousehold(headOfHouseholdParticipantId, createdMembers);

      const headMember = createdMembers.find(m => m.id === headOfHouseholdParticipantId);
      alert(`Successfully created ${headMember?.firstName} ${headMember?.lastName} family with ${createdMembers.length} members!`);
  ...
  const headMember = extractedData.members?.find(m => m.tempId === extractedData.headOfHouseholdId);
  const canCreate = extractedData.members && extractedData.members.length > 0 &&
                    extractedData.headOfHouseholdId &&
                    extractedData.members.every(m => m.firstName && m.lastName);

  const displayFamilyName = customFamilyName ||
    (headMember ? `${headMember.firstName} ${headMember.lastName} Family` : 'Family');
  ...
  const handleCreateFromExisting = () => {
    ...
    const selectedMembers: HouseholdMember[] = selectedParticipantIds.map(participantId => {
      ...
      return {
        ...
        phoneNumber: participant.phoneNumber,
        email: participant.email,
        relationshipToHoH: participantId === headOfHouseholdId ? 'self' : 'other',
        confidence: {},
      };
    });
    ...
    alert(`Successfully created family with ${selectedMembers.length} members!`);
    router.push(`/families/${household.id}`);
  };
    ...
    <PageLayout pageTitle="Create Family">
      <Stack space="400">
        ...
        <Stack space="300">
          ...
          <Text>Create a new family using AI or select existing participants</Text>
        </Stack>
        ...
        {creationMode === 'ai' && (
          <>
            <Text color="subdued">Describe the family in natural language and the AI will help you create it</Text>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                  ...
                  <div
                    style={{
                      ...
                      backgroundColor: message.role === 'user' ? '#7c3aed' :
                                      message.role === 'system' ? '#f3f4f6' : '#f9fafb',
          ...
          <div style={{
            ...
            {!extractedData.members || extractedData.members.length === 0 ? (
              <div style={{
                textAlign: 'center',
                padding: '48px 0',
              }}>
                ...
                <Text variant="sm" color="subdued">Chat with the agent to create a family</Text>
              </div>
            ) : (
              <Stack space="400">
                {headMember && (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      {isEditingFamilyName ? (
                        <div style={{ flex: 1, marginRight: '8px' }}>
                          <TextField
                            value={customFamilyName || `${headMember.firstName} ${headMember.lastName} Family`}
                      ...
                      <Button
                        variant="secondary"
                        size="sm"
                        onPress={() => setIsEditingFamilyName(!isEditingFamilyName)}
                      >
                        {isEditingFamilyName ? 'Done' : 'Edit'}
                      </Button>
                    </div>
                    <Text variant="sm" style={{ color: '#7c3aed' }}>
                      {extractedData.members.length} member{extractedData.members.length !== 1 ? 's' : ''}
                    </Text>
                  </div>
                )}
                  ...
                  {extractedData.members.map((member) => {
                    ...
                    return (
                      <div
                        key={member.tempId}
                        style={{
                          border: member.tempId === extractedData.headOfHouseholdId
                            ? '2px solid #c4b5fd'
                            : '1px solid #e5e7eb',
                          borderRadius: '8px',
                          padding: '16px',
                          backgroundColor: member.tempId === extractedData.headOfHouseholdId
                            ? '#f5f3ff'
                            : 'white',
                        }}
                      >
                        <Stack space="200">
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <InlineStack gap="200" verticalAlign="center">
                              {member.tempId === extractedData.headOfHouseholdId && (
                                <span style={{
                                  padding: '2px 8px',
                              ...
                              {member.tempId !== extractedData.headOfHouseholdId && member.relationshipToHoH && (
                                <span style={{
                                  padding: '2px 8px',
                                  ...
                                  {RELATIONSHIP_LABELS[member.relationshipToHoH]}
                                </span>
                              )}
                            </InlineStack>
                            <Button
                              variant="secondary"
                              size="sm"
                              onPress={() => setEditingMemberId(isEditing ? null : member.tempId)}
                            >
                              {isEditing ? 'Done' : 'Edit'}
                            </Button>
                          </div>
                            ...
                            <Stack space="300">
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <TextField
                                  ...
                                  onChange={(value) => updateMemberField(member.tempId, 'firstName', value)}
                                />
                                <TextField
                                  label="Last Name"
                                  value={member.lastName || ''}
                                  onChange={(value) => updateMemberField(member.tempId, 'lastName', value)}
                                />
                              </div>
                              ...
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => updateMemberField(member.tempId, 'dateOfBirth', e.target.value)}
                                    style={{
                                      width: '100%',
                                      padding: '8px 12px',
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => updateMemberField(member.tempId, 'approximateAge', e.target.value ? parseInt(e.target.value) : undefined)}
                                    style={{
                                      width: '100%',
                                      padding: '8px 12px',
                                      border: '1px solid #d1d5db',
                              ...
                              <div>
                                ...
                                <select
                                  value={member.gender !== undefined ? member.gender : 99}
                                  onChange={(e) => updateMemberField(member.tempId, 'gender', parseInt(e.target.value))}
                                  style={{
                                    width: '100%',
                                    padding: '8px 12px',
                                    border: '1px solid #d1d5db',
                              ...
                              {member.tempId !== extractedData.headOfHouseholdId && (
                                <div>
                                  ...
                                  <select
                                    value={member.relationshipToHoH || 'other'}
                                    onChange={(e) => updateMemberField(member.tempId, 'relationshipToHoH', e.target.value as RelationshipType)}
                                    style={{
                                      width: '100%',
                                      padding: '8px 12px',
                                      border: '1px solid #d1d5db',
                                  ...
                                  >
                                    ...
                                    <option value="foster">Foster Child/Parent</option>
                                    <option value="ward">Ward</option>
                                    <option value="unrelated">Unrelated Household Member</option>
                              ...
                              <TextField
                                ...
                                onChange={(value) => updateMemberField(member.tempId, 'phoneNumber', value)}
                              />

                              <TextField
                                label="Email"
                                value={member.email || ''}
                                onChange={(value) => updateMemberField(member.tempId, 'email', value)}
                              />
                            </Stack>
                          ) : (
                            <>
                              ...
                              <Stack space="100">
                                {(member.dateOfBirth || member.approximateAge) && (
                                ...
                                {member.gender !== undefined && (
                                  <Text variant="sm" color="subdued">
                                    Gender: {HMIS_GENDER_CODES[member.gender]}
                                  </Text>
                                )}
                                {member.phoneNumber && (
                                  <Text variant="sm" color="subdued">Phone: {member.phoneNumber}</Text>
                                )}
                                {member.email && (
                                  <Text variant="sm" color="subdued">Email: {member.email}</Text>
                                )}
                              </Stack>
                            </>
                          )}
          ...
          <Stack space="400">
            <Card>
              <Stack space="400">
                ...
                <Text color="subdued">
                  Choose existing participants to form a family. You must select at least one participant and designate a head of household.
                </Text>
                          ...
                          <Stack space="200">
                            <InlineStack gap="300" verticalAlign="center">
                              ...
                              <Stack space="100">
                                ...
                                {participant.email && (
                                  <Text variant="sm" color="subdued">{participant.email}</Text>
                                )}
                                <Text variant="sm" color="subdued">
                                  Gender: {HMIS_GENDER_CODES[participant.gender]}
                                </Text>
                              </Stack>
                              {isHead && (
                ...
                {selectedParticipantIds.length > 0 && (
                  <Card style={{ backgroundColor: '#f5f3ff' }}>
                    <Stack space="200">
                      ...
                      {headOfHouseholdId && (
                        <Text variant="sm" color="subdued">
                          Head of Household: {participants.find(p => p.id === headOfHouseholdId)?.firstName} {participants.find(p => p.id === headOfHouseholdId)?.lastName}
                        </Text>
                      )}
                      {!headOfHouseholdId && selectedParticipantIds.length > 0 && (
                ...
                <Button
                  ...
                  isDisabled={selectedParticipantIds.length === 0 || !headOfHouseholdId}
                  style={{ width: '100%', backgroundColor: '#10b981' }}
                >
                  Create Family ({selectedParticipantIds.length} member{selectedParticipantIds.length !== 1 ? 's' : ''})
                </Button>
              </Stack>
            </Card>
          </Stack>
