...
export default function RecordServicePage() {
  ...
  const serviceCost = selectedServiceType?.costPerUnit
  ...
  const budgetUtilization = programBudget
    ? (programSpending / programBudget) * 100
    : 0;
  ...
  const handleSubmit = () => {
    ...
    if (enrollment && serviceType.costPerUnit) {
      ...
      if (program && program.budget) {
        ...
        const serviceCost =
          serviceType.costPerUnit * parseFloat(formData.quantity);
                  ...
                  >
                    {participants.map((p) => (
                      <SelectItem key={p.id} id={p.id}>
                        ...
                        {HMIS_GENDER_CODES[p.gender]}, DOB:{' '}
                              ...
                              >
                                <Stack space="200">
                                  ...
                                  >
                                    Program: {program.name}
                                    <br />
                                    Remaining: ${remainingFunds.toFixed(2)} of $
                                    {programBudget.toFixed(2)}
                                    <br />
                                    {!hasSufficientFunds &&
                                      `Need: $${serviceCost.toFixed(2)} (Shortfall: $${(serviceCost - remainingFunds).toFixed(2)})`}
                                  </Text>
                                </Stack>
                              </div>
                            )}
