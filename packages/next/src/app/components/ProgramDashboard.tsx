...
import { ClientLineChart, ClientBarChart, ClientStackedBarChart } from './ClientOnlyCharts';
...
export default function ProgramDashboard() {
  ...
  const { serviceTransactions, serviceTypes, calculateProgramSpending } = useServiceStore();
  ...
  const selectedProgram = currentProgramId ? programs.find(p => p.id === currentProgramId) : null;
  ...
  const activeParticipantIds = new Set(activeEnrollments.map(e => e.participantId));
  ...
  const programHouseholds = currentProgramId
    ? households.filter(h => h.members.some(m => activeParticipantIds.has(m.id)))
    : households;
  const totalHouseholdMembers = programHouseholds.reduce((sum, h) => sum + h.members.length, 0);

  // Services this month - filtered by program enrollments
  const now = new Date();
  const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const enrollmentIds = new Set(activeEnrollments.map(e => e.id));
  const servicesThisMonth = serviceTransactions.filter((t) =>
  ...
  const goalCompletionRate = programGoals.length > 0 ? Math.round((completedGoals / programGoals.length) * 100) : 0;

  // Budget tracking - only shown when program is selected and has a budget
  const programBudget = selectedProgram?.budget;
  const programSpending = currentProgramId ? calculateProgramSpending(currentProgramId) : 0;
  const remainingFunds = programBudget ? programBudget - programSpending : 0;
  const budgetUtilization = programBudget ? Math.round((programSpending / programBudget) * 100) : 0;
  ...
  let enrollmentsByProgram = currentProgramId
    ...
    : programs.map(program => {
        const count = allActiveEnrollments.filter(e => e.programId === program.id).length;
        return {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const count = enrollments.filter(e => {
      ...
      const matchesMonth = enrollDate.getMonth() === date.getMonth() && enrollDate.getFullYear() === date.getFullYear();
      const matchesProgram = currentProgramId ? e.programId === currentProgramId : true;
  ...
  const caseWorkerStats = mockCaseWorkers.map(cw => {
    const caseload = activeEnrollments.filter(e => e.caseWorkerId === cw.id).length;
    return {
    ...
    <Card>
      <Stack space="600">
        ...
        <InlineStack gap="400" verticalAlign="center">
          <Stack space="200">
            <Heading level={2}>
              {selectedProgram ? `${selectedProgram.name} Overview` : 'Program Overview'}
            </Heading>
            <Text variant="sm">
              {selectedProgram
          ...
          {selectedProgram && (
            <Stack space="100">
              ...
              <Text weight="600">{PROGRAM_TYPES[selectedProgram.programType]}</Text>
            </Stack>
          )}
        </InlineStack>

        {/* Key Metrics */}
        <TileLayout columns={selectedProgram && programBudget ? "6" : "5"} columnsSM="2" space="400">
          <Card>
            <Stack space="200">
              <Text variant="sm" color="subdued">Active Enrollments</Text>
              <Heading level={2}>{activeEnrollments.length}</Heading>
              <Text variant="sm">
                {selectedProgram ? `In ${selectedProgram.name}` : `Across ${programs.length} programs`}
              </Text>
            </Stack>
          </Card>
          ...
          <Card>
            <Stack space="200">
              <Text variant="sm" color="subdued">Active Participants</Text>
          ...
          <Card>
            <Stack space="200">
              <Text variant="sm" color="subdued">Families</Text>
              <Heading level={2}>{programHouseholds.length}</Heading>
              <Text variant="sm">
                {totalHouseholdMembers} total members
              </Text>
            </Stack>
          </Card>
          ...
          <Card>
            <Stack space="200">
              <Text variant="sm" color="subdued">Services (This Month)</Text>
              <Heading level={2}>{servicesThisMonth}</Heading>
              <Text variant="sm">{serviceTypes.length} service types available</Text>
            </Stack>
          </Card>

          <Card>
            <Stack space="200">
              <Text variant="sm" color="subdued">Goal Completion</Text>
              <Heading level={2}>{goalCompletionRate}%</Heading>
              <Text variant="sm">{completedGoals} of {programGoals.length} goals</Text>
            </Stack>
          </Card>
          ...
          {selectedProgram && programBudget && (
            <Card>
              <Stack space="200">
                <Text variant="sm" color="subdued">Program Funds</Text>
                <Heading level={2}>
                  ${(remainingFunds / 1000).toFixed(1)}k
                </Heading>
                <Text variant="sm">
                  ${(programSpending / 1000).toFixed(1)}k of ${(programBudget / 1000).toFixed(0)}k spent
                </Text>
                <div style={{
                  width: '100%',
                  height: '8px',
                  ...
                  <div
                    style={{
                      ...
                      height: '100%',
                      backgroundColor: budgetUtilization > 90 ? '#ef4444' : budgetUtilization > 75 ? '#f59e0b' : '#10b981',
        ...
        <Stack space="400">
          ...
          {!selectedProgram && programs.some(p => p.budget && p.budget > 0) && (() => {
            const budgetedPrograms = programs.filter(p => p.budget && p.budget > 0);

            return (
              <Card>
                <ClientStackedBarChart
                  categories={budgetedPrograms.map(p => p.name.length > 20 ? p.name.substring(0, 20) + '...' : p.name)}
                  series={[
                    {
                      name: 'Funds Disbursed',
                      data: budgetedPrograms.map(p => calculateProgramSpending(p.id)),
                      color: '#ef4444',
                    },
                    {
                      name: 'Funds Available',
