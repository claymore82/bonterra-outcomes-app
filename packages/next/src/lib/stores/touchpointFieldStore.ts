...
import { TouchpointCustomField, TOUCHPOINT_FIELD_LIBRARY } from '@/types/touchpointFields';
...
interface TouchpointFieldStore {
  ...
  createField: (field: Omit<TouchpointCustomField, 'id' | 'createdAt' | 'updatedAt'>) => TouchpointCustomField;
...
const initialFields: TouchpointCustomField[] = TOUCHPOINT_FIELD_LIBRARY.map((field, index) => ({
  ...field,
  id: `field-${index + 1}`,
  createdAt: new Date(),
  updatedAt: new Date(),
...
export const useTouchpointFieldStore = create<TouchpointFieldStore>((set, get) => ({
  ...
  initializeFromLibrary: () => {
    const libraryFields: TouchpointCustomField[] = TOUCHPOINT_FIELD_LIBRARY.map((field, index) => ({
      ...field,
      id: `field-${index + 1}`,
      createdAt: new Date(),
      updatedAt: new Date(),
