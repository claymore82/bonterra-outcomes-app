...
export default function IntakeAgentPage() {
              ...
              if (line.startsWith('data: ')) {
                try {
                  ...
                  if (data.type === 'token') {
                    ...
                    const displayContent = assistantMessage.content.replace(/```json\n[\s\S]*?\n```/g, '').trim();

                    setMessages((prev) => {
                      const newMessages = [...prev];
                      const lastMessage = newMessages[newMessages.length - 1];
                      const displayMessage = { ...assistantMessage, content: displayContent };
  ...
  const handleDocumentExtract = useCallback((data: any) => {
    ...
    const confirmationMessage: Message = {
      id: Date.now().toString(),
      role: 'system',
      content: 'Document data extracted successfully! Please review the information on the right.',
      timestamp: new Date(),
    };
    setMessages((prev) => [...prev, confirmationMessage]);
  }, []);

  const handleCSVExtract = useCallback((data: { headers: string[]; rows: any[]; autoMapping: Record<string, string>; isFamilyCSV?: boolean }) => {
  ...
  }, [csvParticipants, currentProgramId, currentSiteId, currentTenantId, createParticipant, createEnrollment, router]);
      ...
      if (enrolleeType === 'participant') {
        ...
        const household = createHousehold({
          ...
          members: createdParticipantIds.map((id, idx) => {
            ...
            return {
              id,
              relationshipToHoH: member.relationshipToHoH || (idx === 0 ? 'self' : 'other'),
            };
          }),
          headOfHouseholdId: extractedData.headOfHouseholdId || createdParticipantIds[0],
        ...
        createEnrollment({
          ...
          enrollmentDate: extractedData.enrollmentDate ? new Date(extractedData.enrollmentDate) : new Date(),
          status: 'active',
        });

        alert(`Successfully enrolled family with ${createdParticipantIds.length} members in ${extractedData.program}`);
        ...
        alert(`Successfully enrolled ${entity.name} in ${extractedData.program}`);
  ...
  const handleEnrollNow = () => {
    if (!createdParticipantId || !extractedData.programId || !extractedData.caseWorkerId) return;
  ...
  const requiredFields = ['firstName', 'lastName', 'dateOfBirth_or_age', 'program', 'caseWorker'];
  ...
  return (
    <PageLayout pageTitle="Intake Agent">
      <Stack space="400">
        ...
        {showEnrollmentOption && createdParticipantId && (
          ...
          >
            <Stack space="400" align="center">
              ...
              <Text color="subdued">
                {extractedData.firstName} {extractedData.lastName} has been added to the system.
              </Text>
              <div style={{ width: '100%', maxWidth: '600px', padding: '20px', backgroundColor: '#f9fafb', borderRadius: '8px' }}>
                <Text weight="500" style={{ marginBottom: '8px' }}>What would you like to do next?</Text>
              </div>
              <InlineStack gap="300">
          ...
          <Card>
            <Stack space="400">
                    ...
                    >
                      ...
                      {isFamilyCSV && (
                        <>
                          ...
                          <option value="relationship">Relationship to Head of Household</option>
                        </>
                      )}
              ...
              <div style={{
                ...
                <Text variant="sm">
                  💡 <strong>Tip:</strong> Columns are auto-detected, but you can adjust the mapping as needed. Fields marked "Skip this column" will be ignored.
                </Text>
              </div>
              ...
              <InlineStack gap="300">
                ...
                <Button
                  ...
                  onPress={() => {
                    ...
                    const initialValueMapping: Record<string, Record<string, any>> = {};
                    ...
                    Object.entries(columnMapping).forEach(([csvCol, sysField]) => {
                      ...
                      if (sysField === 'gender') {
                        ...
                        uniqueValues.forEach(val => {
                          ...
                          if (lower.includes('f') || lower === 'woman' || lower === 'girl' || lower === 'female') {
                            genderMap[val] = 0;
                          } else if (lower.includes('m') || lower === 'man' || lower === 'boy' || lower === 'male') {
                            genderMap[val] = 1;
                          } else if (lower.includes('trans')) {
                            genderMap[val] = 2;
                          } else if (lower.includes('non') || lower.includes('nb') || lower.includes('enby')) {
                            genderMap[val] = 3;
                          } else {
                    ...
                    if (fieldsNeedingValueMapping.length > 0) {
                      ...
                      if (isFamilyCSV) {
                        ...
                        csvRawData.forEach(row => {
                          ...
                          const familyName = member.familyName || 'Unknown Family';
                        ...
                        const familyList = Object.entries(families).map(([name, members]) => ({
                          familyName: name,
                          members,
                        }));
          ...
          <Card>
            <Stack space="400">
                  ...
                  <div key={csvCol}>
                    <div style={{
                      ...
                      <Text weight="600" style={{ marginBottom: '12px' }}>
                        {csvCol} → {systemField === 'gender' ? 'Gender' : systemField}
                      </Text>
                      ...
                      <div style={{
                        ...
                        <Text variant="sm" weight="600">CSV Value</Text>
                        <Text variant="sm" weight="600">System Value</Text>

                        {Object.entries(mappings).map(([csvValue, sysValue]) => (
              ...
              <div style={{
                ...
                <Text variant="sm">
                  💡 <strong>Tip:</strong> Auto-detected mappings are shown, but you can adjust them. For example, map "M" to "Male" or "Trans" to "Transgender".
                </Text>
              </div>
                  ...
                  onPress={() => {
                    ...
                    csvRawData.forEach(row => {
                      ...
                      Object.entries(columnMapping).forEach(([csvCol, sysField]) => {
                        ...
                        if (valueMapping[csvCol] && valueMapping[csvCol][row[csvCol]] !== undefined) {
                          participant[sysField] = valueMapping[csvCol][row[csvCol]];
                        } else {
                          participant[sysField] = row[csvCol];
                        }
                      });
            ...
            <Stack space="400">
                        ...
                        <Stack space="300">
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{
                              ...
                              <Text variant="sm" weight="600" style={{ color: '#1e40af' }}>
                            ...
                            {family.members.map((member: any, mIdx: number) => (
                              <div
                                ...
                                style={{
                                  ...
                                  gap: '16px',
                                  padding: '8px 0',
                                  borderBottom: mIdx < family.members.length - 1 ? '1px solid #f3f4f6' : 'none',
                                }}
                              >
                                <Text variant="sm" style={{ minWidth: '150px' }}>
                                  <strong>{member.firstName} {member.lastName}</strong>
                                </Text>
                                <Text variant="sm" color="subdued" style={{ minWidth: '120px' }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ backgroundColor: '#f9fafb', position: 'sticky', top: 0 }}>
                      <tr>
                        ...
                        <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>
                          <Text variant="sm" weight="600">DOB</Text>
                        </th>
                        <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>
                          <Text variant="sm" weight="600">Email</Text>
                        </th>
                        <th style={{ padding: '12px', textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>
                          <Text variant="sm" weight="600">Gender</Text>
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      return (
                        <tr key={idx}>
                          ...
                          <td style={{ padding: '12px', borderBottom: '1px solid #e5e7eb' }}>
                            <Text variant="sm">
                              {p.gender !== undefined ? genderLabels[p.gender] : '-'}
                            </Text>
                          </td>
                        </tr>
                      );
              ...
              {!currentProgramId ? (
                <div style={{
                  ...
                  <Stack space="300">
                    <InlineStack gap="200" verticalAlign="center">
                      ...
                      <Text weight="600" style={{ color: '#dc2626', fontSize: '16px' }}>
                        Program Not Selected
                      </Text>
                    </InlineStack>
                    <Text variant="sm" style={{ color: '#991b1b' }}>
                      All participants will be enrolled in a program. Please select a program from the <strong>top navigation bar</strong> before importing.
                    </Text>
                    <div style={{
                      padding: '12px',
                      ...
                      <Text variant="sm" style={{ color: '#92400e' }}>
                        👆 Look for <strong>"All Programs"</strong> dropdown in the header (top right corner) and select your program.
                      </Text>
                    </div>
                  </Stack>
                </div>
                ...
                <div style={{
                  ...
                  <Text variant="sm">
                    ✓ All participants will be enrolled in: <strong>{programs.find(p => p.id === currentProgramId)?.name}</strong>
                  </Text>
                </div>
              )}
              ...
              <InlineStack gap="300">
                ...
                >
                  {isSubmitting ? 'Importing...' : `Import ${csvParticipants.length} Participants`}
                </Button>
              </InlineStack>
            </Stack>
          </Card>
        ...
        {/* AI Chat Interface */}
        {enrolleeType && !showEnrollmentOption && !showBulkImport && !showColumnMapping && !showValueMapping && (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <Text variant="sm" color="subdued">
                    Chat with the intake agent to enroll a {enrolleeType === 'participant' ? 'participant' : enrolleeType === 'family' ? 'family' : 'organization'}
                  </Text>
                </div>
                <Button
                  variant="primary"
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', borderBottom: '1px solid #e5e7eb' }}>
                  <button
                    ...
                    style={{
                      padding: '8px 16px',
                      border: 'none',
                      borderBottom: uploadType === 'document' ? '2px solid #7c3aed' : '2px solid transparent',
                      backgroundColor: 'transparent',
                      cursor: 'pointer',
                      fontWeight: uploadType === 'document' ? '600' : '400',
                      color: uploadType === 'document' ? '#7c3aed' : '#6b7280',
                    }}
                  >
                    📄 Document
                  </button>
                  <button
                    ...
                    style={{
                      padding: '8px 16px',
                      border: 'none',
                      borderBottom: uploadType === 'csv' ? '2px solid #7c3aed' : '2px solid transparent',
                      backgroundColor: 'transparent',
                      cursor: 'pointer',
          ...
          >
            <div style={{ marginBottom: '24px' }}>
              ...
              {/* Progress Bar */}
              {(extractedData.firstName || extractedData.lastName || extractedData.program) && (
            ...
            {isEditing ? (
              <Stack space="300">
                  ...
                  <Stack space="200">
                    <Select
                      ...
                      onSelectionChange={(key) => {
                        const program = availablePrograms.find((p) => p.id === key);
                    ...
                    <Select
                      ...
                      onSelectionChange={(key) => {
                        ...
                        updateField('caseWorker', cw ? `${cw.firstName} ${cw.lastName}` : '');
                      }}
                    >
                ...
                <Button variant="secondary" onPress={() => setIsEditing(false)} style={{ width: '100%' }}>
                  Done Editing
                </Button>
              </Stack>
            ) : (
                        ...
                        {(extractedData.firstName || extractedData.lastName) && (
                          ...
                          >
                            ...
                            <Text weight="600" style={{ fontSize: '18px' }}>
                              {extractedData.firstName || '[First]'} {extractedData.lastName || '[Last]'}
                            </Text>
                          </div>
                        )}
                      </Stack>
                  ...
                  <div>
                    ...
                    <Stack space="300">
                      <div>
                        ...
                        <Text variant="sm" color={extractedData.program ? 'default' : 'subdued'}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
                          ...
                          {getConfidenceBadge('caseWorker', extractedData.confidence)}
                        </div>
                        <Text variant="sm" color={extractedData.caseWorker ? 'default' : 'subdued'}>
                          {extractedData.caseWorker || <em>Not provided</em>}
                        </Text>
                      </div>
                    </Stack>
                  </div>
                    ...
                    <Stack space="300">
                      <div>
                        ...
                        {extractedData.dateOfBirth ? (
                          <Text variant="sm">{new Date(extractedData.dateOfBirth).toLocaleDateString()}</Text>
                      ...
                      <div>
                        ...
                        <Text
                          variant="sm"
                          color={
                            extractedData.gender !== undefined && extractedData.gender !== 99 ? 'default' : 'subdued'
                          }
                        >
