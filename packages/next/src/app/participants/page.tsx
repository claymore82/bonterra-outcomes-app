...
export default function ParticipantsPage() {
  ...
  const getParticipantStatus = (participantId: string) => {
    ...
    const participantEnrollments = activeEnrollments.filter(
  ...
  const filteredParticipants = useMemo(() => {
    ...
    return participants.filter((participant) => {
      ...
      if (currentProgramId && currentProgramId !== '') {
        matchesProgram = participantEnrollments.some(
  ...
  const activeCount = participants.filter(
    (p) => getParticipantStatus(p.id) === 'active',
  ).length;
  const inactiveCount = participants.filter(
    (p) => getParticipantStatus(p.id) === 'inactive',
  ).length;
    ...
    <PageLayout pageTitle="Individuals">
      <Stack space="600">
        ...
        <InlineStack gap="400" distribute="end">
          ...
          <Button variant="primary" onPress={() => router.push('/intake')}>
            <Icon name="sparkles" />
            Intake Agent
          </Button>
        </InlineStack>
        ...
        <Card>
          <Stack space="400">
            ...
            >
              <Heading level={3}>
                {filteredParticipants.length} Individual
            ...
            {filteredParticipants.length === 0 ? (
              ...
              <Stack space="300">
                {filteredParticipants.map((participant) => {
                  ...
                  const programsText =
                    getParticipantPrograms(participant.id) || 'None';
                      ...
                      >
                          ...
                          <InlineStack gap="300" verticalAlign="center">
                            ...
                            <Stack space="100">
                              <InlineStack gap="200" verticalAlign="center">
                                ...
                                <SimpleBadge
                                  tone={
                                    status === 'active' ? 'positive' : 'neutral'
                                  }
                                >
                        ...
                        <InlineStack gap="200">
                          ...
                          <Button
                            ...
                            onPress={() =>
                              router.push(
                                `/participants/${participant.id}/record-service`,
                              )
                            }
                          >
                          ...
                          <Button
                            ...
                            onPress={() =>
                              router.push(
                                `/enroll?participantId=${participant.id}`,
                              )
                            }
                          >
                            <Icon name="user-plus" />
