...
import { CustomField, FIELD_TYPE_LABELS, FieldType } from '@/types/customFields';
import { STANDARD_SOURCES, StandardDemographicField } from '@/lib/standardDemographics';
import PageLayout from '../../components/PageLayout';

export default function DemographicsPage() {
  const { customFields, addCustomField, updateCustomField, deleteCustomField } = useCustomFieldStore();
  const { programs } = useProgramStore();
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [showImportModal, setShowImportModal] = useState(false);
  const [editingField, setEditingField] = useState<CustomField | null>(null);
  const [selectedSource, setSelectedSource] = useState<string | null>(null);
  const [selectedFieldsForImport, setSelectedFieldsForImport] = useState<Set<string>>(new Set());
  ...
  const handleSave = () => {
    ...
    const options = (formData.fieldType === 'dropdown' || formData.fieldType === 'multi-select')
    ...
    const fieldData = {
      ...
      profiles: formData.profiles.length > 0 ? formData.profiles as any : ['general' as const],
    };
  ...
  const handleImportFields = () => {
    ...
    const fieldsToImport = source.fields.filter(f => selectedFieldsForImport.has(f.name));
    let importedCount = 0;
    let skippedCount = 0;
    ...
    alert(`Imported ${importedCount} field(s). ${skippedCount > 0 ? `Skipped ${skippedCount} duplicate(s).` : ''}`);
    ...
    <PageLayout pageTitle="Demographic Fields">
      <Stack space="600">
        ...
        <div>
          ...
          <Heading level={1} style={{ marginBottom: '8px' }}>Demographic Fields</Heading>
          <Text color="subdued">Configure demographics and map to government standards</Text>
        </div>
                    ...
                    >
                      ...
                      <td style={{ padding: '16px 24px' }}>
                        <div style={{ display: 'flex', gap: '12px', fontSize: '14px' }}>
                          <button
                            onClick={() => alert('Map functionality coming soon')}
                            style={{
                              background: 'none',
                              border: 'none',
                              padding: 0,
        ...
        {showCreateModal && (
          ...
          >
            ...
            >
              <Stack space="400">
                <Heading level={3}>{editingField ? 'Edit' : 'Create'} Custom Field</Heading>
                ...
                <div>
                  ...
                  >
                    <option value="individual">Individual (person-level)</option>
                    <option value="household">Household (family attributes)</option>
                    <option value="entity">Entity (organization attributes)</option>
                    <option value="all">All (applies to everyone)</option>
                  </select>
                  <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
                    Who does this field describe? Individual = person attributes, Household = family attributes
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {[
                      ...
                      <label key={profile.value} style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                        <input
                          type="checkbox"
                  ...
                  <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '8px' }}>
                    Select which standards this field follows (e.g., HMIS has specific gender codes)
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, hmisCompliant: e.target.checked })}
                      style={{ marginRight: '8px' }}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, visibleInIntake: e.target.checked })}
                      style={{ marginRight: '8px' }}
                    />
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, visibleInProfile: e.target.checked })}
                      style={{ marginRight: '8px' }}
                    />
                ...
                <div>
                  <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', marginBottom: '12px' }}>
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, programSpecific: e.target.checked, programIds: [] })}
                      style={{ marginRight: '8px' }}
                    />
                    <span style={{ fontSize: '14px', fontWeight: 500 }}>Program-specific field</span>
                  </label>
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {programs.filter(p => p.status === 'active').map((program) => (
                          <label key={program.id} style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                            <input
                              type="checkbox"
                    ...
                    <div>
                      ...
                      {formData.fieldType === 'text' && (
                        <input
                          type="text"
                          disabled
                          placeholder={formData.helpText || `Enter ${formData.label.toLowerCase()}`}
                          style={{
                            width: '100%',
                            padding: '8px 12px',
                            border: '1px solid #d1d5db',
                      ...
                      {formData.fieldType === 'checkbox' && (
                        <label style={{ display: 'flex', alignItems: 'center', cursor: 'not-allowed' }}>
                          <input
                            ...
                            style={{ marginRight: '8px' }}
                          />
                          <span style={{ fontSize: '14px' }}>{formData.label}</span>
                        </label>
                      )}
                      {formData.helpText && (
                        <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '4px' }}>
                ...
                <InlineStack space="200">
                  ...
                  <Button variant="secondary" onPress={() => { setShowCreateModal(false); resetForm(); }}>
                    Cancel
                  </Button>
                </InlineStack>
              </Stack>
            ...
            >
              <Stack space="400">
                <div>
                  ...
                  <Text color="subdued">Select a standard source and choose fields to import</Text>
                </div>
                    ...
                    {STANDARD_SOURCES.map(source => (
                      ...
                      >
                        <Stack space="200">
                          ...
                          <Text variant="sm" color="subdued">{source.description}</Text>
                          <Text variant="sm" weight="500" style={{ color: '#7c3aed' }}>
                      ...
                      return (
                        <Stack space="400">
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <Text weight="500">
                              {selectedFieldsForImport.size} of {source.fields.length} fields selected
                            </Text>
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <Button
                                ...
                                size="sm"
                                onPress={() => {
                                  const allFieldNames = new Set(source.fields.map(f => f.name));
                              ...
                              <Button
                                variant="secondary"
                                size="sm"
                                onPress={() => setSelectedFieldsForImport(new Set())}
                              >
                                Clear All
                              </Button>
                            </div>
                          ...
                          <div style={{
                            ...
                            {source.fields.map((field, idx) => {
                              const exists = customFields.some(cf => cf.name === field.name);
                              const isSelected = selectedFieldsForImport.has(field.name);
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    padding: '16px',
                                    borderBottom: idx < source.fields.length - 1 ? '1px solid #e5e7eb' : 'none',
                                    backgroundColor: exists ? '#fef3c7' : (isSelected ? '#f5f3ff' : 'white'),
                                    opacity: exists ? 0.6 : 1,
                                  }}
                                >
                                  <label style={{ display: 'flex', cursor: exists ? 'not-allowed' : 'pointer' }}>
                                    <input
                                      type="checkbox"
                                      checked={isSelected}
                                      disabled={exists}
                                      onChange={() => !exists && toggleFieldForImport(field.name)}
                ...
                <InlineStack space="200" distribute="end">
                  ...
                  {selectedSource && (
                    ...
                    >
                      Import {selectedFieldsForImport.size} Field{selectedFieldsForImport.size !== 1 ? 's' : ''}
                    </Button>
                  )}
                </InlineStack>
              </Stack>
