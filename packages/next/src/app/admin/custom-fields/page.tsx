...
import { CustomField, FieldType, FIELD_TYPE_LABELS, FieldAppliesTo } from '@/types/customFields';
...
export default function CustomFieldsPage() {
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      ...
      addCustomField(formData as Omit<CustomField, 'id' | 'createdAt' | 'updatedAt' | 'order'>);
    }
  ...
  const needsOptions = formData.fieldType === 'dropdown' || formData.fieldType === 'multi-select';
    ...
    <PageLayout pageTitle="Custom Fields">
      <Stack space="600">
        ...
        {isCreating && (
          <Card>
            <Stack space="500">
              <Heading level={2}>{editingId ? 'Edit Field' : 'Create New Field'}</Heading>
              ...
              <Stack space="400">
                ...
                <InlineStack gap="400">
                  <Select
                    ...
                    onSelectionChange={(key) => setFormData({ ...formData, fieldType: key as FieldType })}
                  >
                  ...
                  <Select
                    ...
                    onSelectionChange={(key) => setFormData({ ...formData, appliesTo: key as FieldAppliesTo })}
                  >
                    <SelectItem id="individual">Individual (person-level)</SelectItem>
                    <SelectItem id="household">Household (family-level)</SelectItem>
                    <SelectItem id="entity">Entity (organization-level)</SelectItem>
                ...
                {needsOptions && (
                  <TextField
                    ...
                    onChange={(value) => setFormData({ ...formData, options: value.split('\n').filter(o => o.trim()) })}
                ...
                <Stack space="300">
                  ...
                  <InlineStack gap="400">
                    ...
                    <Checkbox
                      ...
                      onChange={(isSelected) => setFormData({ ...formData, visibleInIntake: isSelected })}
                    >
                      <Text variant="sm">Show in Intake</Text>
                    </Checkbox>
                    <Checkbox
                      isSelected={formData.visibleInProfile || false}
                      onChange={(isSelected) => setFormData({ ...formData, visibleInProfile: isSelected })}
                    >
                      <Text variant="sm">Show in Profile</Text>
                    </Checkbox>
                    <Checkbox
                      isSelected={formData.hmisCompliant || false}
                      onChange={(isSelected) => setFormData({ ...formData, hmisCompliant: isSelected })}
                    >
                ...
                <Stack space="300">
                  <Checkbox
                    ...
                    onChange={(isSelected) => setFormData({ ...formData, programSpecific: isSelected, programIds: isSelected ? formData.programIds : [] })}
                  >
                    ...
                    <Card>
                      <Stack space="300">
                        {programs.map((program) => (
                          <Checkbox
                            key={program.id}
                            isSelected={formData.programIds?.includes(program.id) || false}
                            onChange={(isSelected) => {
        ...
        <Card>
          <Stack space="400">
            <Heading level={2}>Configured Fields ({customFields.length})</Heading>

            {customFields.length === 0 ? (
              <Text>No custom fields configured yet. Click &quot;Create Custom Field&quot; to get started.</Text>
            ) : (
              <Stack space="300">
                {customFields.map((field) => (
                  <Card key={field.id}>
                    <Stack space="300">
                      ...
                      <InlineStack gap="200">
                        <Text variant="sm" color="subdued">Type: {FIELD_TYPE_LABELS[field.fieldType]}</Text>
                        {field.required && (
                          <Text variant="sm" style={{ color: '#dc2626' }}>Required</Text>
                        )}
                        {field.hmisCompliant && (
                          <Text variant="sm" style={{ color: '#2563eb' }}>HMIS</Text>
                        )}
                        {field.visibleInIntake && (
                          <Text variant="sm" style={{ color: '#16a34a' }}>Intake</Text>
                        )}
                        {field.visibleInProfile && (
                          <Text variant="sm" style={{ color: '#7c3aed' }}>Profile</Text>
                        )}
                      </InlineStack>
